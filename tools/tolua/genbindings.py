...
def main():
    ...
    # NOTICE THIS, ndk-r9b
    ndk_root = os.path.join(ndk_root, '../ndk-r9b')
    ...
    if platform == 'win32':
        x86_llvm_path = os.path.abspath(
            os.path.join(ndk_root, 'toolchains/llvm-3.4/prebuilt',
                         '%s' % cur_platform))
    ...
    touhou_game_root = os.path.abspath(
        os.path.join(os.path.dirname(__file__), '..', '..'))
    project_root = os.path.abspath(os.path.join(touhou_game_root, 'cocos2d'))
    cocos_root = os.path.abspath(os.path.join(project_root, ''))
    cxx_generator_root = os.path.abspath(
        os.path.join(touhou_game_root, 'tools/bindings-generator'))
    ...
    # To fix parse error on windows,
    # we must difine __WCHAR_MAX__ and undefine __MINGW32__ .
    if platform == 'win32':
        config.set('DEFAULT', 'extra_flags',
    ...
    try:
        tolua_root = '%s/tools/tolua' % touhou_game_root
        output_dir = '%s/Classes/LuaBindings' % touhou_game_root
        cmd_args = {
            'conversation_layer.ini':
            ('conversation_layer', 'lua_conversation_layer'),
        }
        target = 'lua'
        ...
        for key in cmd_args.keys():
            ...
            print 'Generating bindings for %s ......' % (key)
            command = '%s %s %s -s %s -t %s -o %s -n %s' % (python_bin,
                                                            generator_py, cfg,
                                                            args[0], target,
                                                            output_dir,
                                                            args[1])
