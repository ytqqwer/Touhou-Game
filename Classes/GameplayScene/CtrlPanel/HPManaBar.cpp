...
HpManaBar*
HpManaBar::create(const std::string& tag, const int maxHpValue, const int maxManaValue)
{
...
HpManaBar::init()
{
    ...
    _manaBarBlack->setContentSize(
        Size(_manaBar->getContentSize().width, _manaBar->getContentSize().height * 1.10));
    ...
    Director::getInstance()->getEventDispatcher()->addCustomEventListener(
        "mana_change", [this](EventCustom* e) {
            ...
            if (this->_tag == change->tag) {
                ...
                _manaStencil->setContentSize(
                    Size(this->_manaBar->getContentSize().width * currentMpPercent,
                         this->_manaBar->getContentSize().height));
            }
        });
